...
from argent_contract.contracts.Upgradable import _set_implementation
...
@storage_var
func _latest_tx_timestamp() -> (res : felt):
end
...
func initialize{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    # indicate account initialization
    let (timestamp) = get_block_timestamp()
    _latest_tx_timestamp.write(timestamp)
...
func __execute__{
    ...
    if calls_len == 1:
        if calls[0].to == tx_info.account_contract_address:
            ...
            if signer_condition == 0:
                ...
                validate_signer_signature(
                    tx_info.transaction_hash, tx_info.signature, tx_info.signature_len
                )
                jmp do_execute
            end
            if guardian_condition == 0:
                # validate guardian signature
                validate_guardian_signature(
                    tx_info.transaction_hash, tx_info.signature, tx_info.signature_len
                )
                jmp do_execute
            end
    ...
    validate_guardian_signature(
        tx_info.transaction_hash, tx_info.signature + 2, tx_info.signature_len - 2
    )
...
@view
func get_latest_tx_timestamp{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ) -> (timestamp : felt):
    let (res) = _latest_tx_timestamp.read()
...
func validate_signer_signature{
    syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, ecdsa_ptr : SignatureBuiltin*, range_check_ptr
}(message : felt, signatures : felt*, signatures_len : felt) -> (is_valid : felt):
